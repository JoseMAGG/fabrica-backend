...
import com.example.Fabrica_gr6.repositories.DocenteRepository;
...
public class DocenteService {
    ...
    public ResponseEntity<Docente> update(@RequestBody Docente docente){
        if(docente.getIdPersona()==null){ //no le mande un id
            return ResponseEntity.badRequest().build();
        }
        if(!docenteRepository.existsById(docente.getIdPersona())){ //si el Id NO existe (NUMEO MUY GRANDE)
            return ResponseEntity.notFound().build();
        }
        //de lo contrario
        Docente result=docenteRepository.save(docente);
