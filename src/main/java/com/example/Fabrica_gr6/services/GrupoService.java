...
import com.example.Fabrica_gr6.repositories.GrupoRepository;
...
public class GrupoService {
    ...
    public ResponseEntity<Grupo> update(@RequestBody Grupo grupo){
        if(grupo.getBloque()==null){ //no le mande un id
            return ResponseEntity.badRequest().build();
        }
        if(!grupoRepository.existsById(grupo.getBloque())){ //si el Id NO existe (NUMEO MUY GRANDE)
            return ResponseEntity.notFound().build();
        }
        //de lo contrario
        Grupo result= grupoRepository.save(grupo);
